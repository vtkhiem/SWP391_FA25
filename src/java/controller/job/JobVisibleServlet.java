...
@WebServlet(name = "JobVisiblizeServlet", urlPatterns = {"/visible_job"})
public class JobVisibleServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (employer.getEmployerId() != job.getEmployerID()) {
                session.setAttribute("error", "Bạn không có quyền hiện công việc này.");
                response.sendRedirect(request.getContextPath() + "/employer_jobs");
                return;
            }

            boolean visible = jobPostDAO.visibleJobPost(jobId);

            if (visible) {
                session.setAttribute("message", "Hiện công việc thành công!");
            } else {
                session.setAttribute("error", "Hiện công việc thất bại. Vui lòng thử lại.");
            }
