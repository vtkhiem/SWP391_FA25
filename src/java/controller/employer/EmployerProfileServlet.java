...
// <editor-fold defaultstate="collapsed" desc="HttpServlet methods. Click on the + sign on the left to edit the code.">
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ServiceEmployer serviceEmployer = dao.getByEmployerAndService(employer.getEmployerId(), sid);

            request.setAttribute("service", s);
            request.setAttribute("serviceEmployer", serviceEmployer);
            request.setAttribute("employer", employer);

            // Forward tới trang hồ sơ nhà tuyển dụng
            request.getRequestDispatcher("profile.jsp").forward(request, response);

        } catch (SQLException ex) {
            ex.printStackTrace();
            request.setAttribute("error", "Lỗi khi tải thông tin nhà tuyển dụng.");
            request.getRequestDispatcher("profile.jsp").forward(request, response);
        }
    }
