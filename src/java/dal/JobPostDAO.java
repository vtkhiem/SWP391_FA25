...
public class JobPostDAO extends DBContext {
    ...
    public List<JobPost> searchJobs(String category, String location,
        ...
        if (category != null && !category.isEmpty()) sql.append(" AND Category LIKE ?");
        if (location != null && !location.isEmpty()) sql.append(" AND Location LIKE ?");
        if (minSalary != null) sql.append(" AND OfferMin >= ?");
        if (maxSalary != null) sql.append(" AND OfferMax <= ?");
        ...
        try (PreparedStatement ps = c.prepareStatement(sql.toString())) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) list.add(extractJobPost(rs));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    private JobPost extractJobPost(ResultSet rs) throws SQLException {
        ...
        if (ts != null) job.setDayCreate(ts.toLocalDateTime());
        return job;
    }
