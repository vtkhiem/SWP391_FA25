...
<%
    ...
    model.Candidate cand = (model.Candidate) request.getAttribute("candidate");
...
<style>
    ...
    .btn.link { background:transparent; border:none; color:#003366; text-decoration:underline; padding:0; }
...
<div class="wrap">
    <div class="head">
        <div class="title">Ban Ứng Viên</div>
        <a class="btn" href="<c:url value='/admin/candidates'/>">← Quay lại danh sách</a>
    </div>
    <div class="content">
        ...
        <div class="row">
            ...
            <div class="val">${candidate.candidateId}</div>
        </div>
        <div class="row">
            <div class="label">Họ tên</div>
            <div class="val">${candidate.candidateName}</div>
        </div>
        <div class="row">
            <div class="label">Email</div>
            <div class="val">${candidate.email}</div>
        </div>
        <div class="row">
                ...
                <c:choose>
                    <c:when test="${not empty activeBan}">
                        <span class="status warn">
                            ĐANG BỊ CẤM
                            <c:choose>
                                <c:when test="${activeBan.bannedUntil == null}">
                                    (Vĩnh viễn)
                                </c:when>
                                <c:otherwise>
                                    (Đến: 
                                    <%
                                        ...
                                        if (activeBan != null && activeBan.getBannedUntil() != null) {
                                            java.time.ZonedDateTime z =
                                                activeBan.getBannedUntil().atZone(java.time.ZoneId.of("UTC"))
                                                         .withZoneSameInstant(java.time.ZoneId.of("Asia/Bangkok"));
                                            String human = z.toLocalDate().toString() + " " + z.toLocalTime().toString();
                                            out.print(human + " (GMT+7)");
                                        }
                                    %>
                                    )
                                </c:otherwise>
                    ...
                    <c:otherwise>
                        <span class="status ok">KHÔNG BỊ CẤM</span>
                    </c:otherwise>
                </c:choose>
            </div>
        </div>
        ...
        <div class="form-card">
            <form method="post" action="<c:url value='/admin/candidates/ban'/>" onsubmit="return validateBanForm();">
                <input type="hidden" name="candidateId" value="${candidate.candidateId}">
                <input type="hidden" name="backTo" value="${param.backTo}">

                <div class="form-row">
                    <label>
                        <input type="radio" name="banType" value="temp" checked onclick="togglePicker(true)"> Cấm có thời hạn
                    </label>
                    &nbsp;&nbsp;
                    <label>
                        <input type="radio" name="banType" value="permanent" onclick="togglePicker(false)"> Cấm vĩnh viễn
                    </label>
                </div>
